...
function generateRandomData(num) {
    const foodTypes = ["Chinese", "Thai", "Indian", "Pizza", "Grill", "Sushi", "African", "Curry"];
    const outcodes = ["CF24", "SS9", "W6", "CH4", "S65", "E11", "NE16", "TW5", "B92", "NW1", "SW9"];
    const ratings = [3, 4, 4.5, 5, 5.5, 6];

    const data = [];

    for (let i = 0; i < num; i++) {
        const id = `55f14312c7447c3da7051b${(i + 26).toString(16)}`;  // Random _id generation (hex)
        const foodType = foodTypes[Math.floor(Math.random() * foodTypes.length)];
        const outcode = outcodes[Math.floor(Math.random() * outcodes.length)];
        const rating = ratings[Math.floor(Math.random() * ratings.length)];
        const name = `Restaurant ${i + 1}`;
        const address = `${Math.floor(Math.random() * 100) + 1} Random Street`;
        const postcode = `${Math.floor(Math.random() * 10) + 1}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${Math.floor(Math.random() * 10) + 1}`;
        
        data.push({
            URL: `http://www.just-eat.co.uk/restaurants-${name.replace(/\s+/g, '-').toLowerCase()}/menu`,
            _id: id,
            address: address,
            "address line 2": "City",
            name: name,
